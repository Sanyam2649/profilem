...
export async function createProfile(userId, data) {
  ...
  const profile = new Profile({ ...data, user: new Types.ObjectId(userId) });
  await profile.save();
  return profile;
}
...
export async function deleteProfileField(profileId, field, itemIdOrValue) {
  ...
  const isArrayOfStrings = field === 'interests' || field === 'certifications';

  const pullQuery = isArrayOfStrings
    ? { $pull: { [field]: itemIdOrValue } }
    : {
        $pull: {
